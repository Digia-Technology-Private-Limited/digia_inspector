...
class ActionSearchBar extends StatefulWidget {
  const ActionSearchBar({
    super.key,
    this.initialQuery = '',
    this.currentFilter = ActionStatusFilter.all,
    required this.onSearchChanged,
    required this.onFilterChanged,
  });

  final String initialQuery;
  final ActionStatusFilter currentFilter;
  final ValueChanged<String> onSearchChanged;
  final ValueChanged<ActionStatusFilter> onFilterChanged;
...
class _ActionSearchBarState extends State<ActionSearchBar> {
  ...
  Widget _buildFilterButton() {
    final hasActiveFilter = widget.currentFilter != ActionStatusFilter.all;
  ...
  void _showFilterBottomSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (context) => ActionFilterBottomSheet(
...
/// Filter bottom sheet for action status
class ActionFilterBottomSheet extends StatefulWidget {
  const ActionFilterBottomSheet({
    super.key,
  ...
  @override
  State<ActionFilterBottomSheet> createState() =>
      _ActionFilterBottomSheetState();
}

class _ActionFilterBottomSheetState extends State<ActionFilterBottomSheet> {
  late ActionStatusFilter _selectedFilter;
          ...
          Row(
            children: [
              ...
              TextButton(
                onPressed: () {
                  setState(() {
                    _selectedFilter = ActionStatusFilter.all;
                  });
                },
                child: Text(
                  'Clear',
  ...
  Widget _buildFilterOption(ActionStatusFilter filter) {
    ...
    return Container(
      ...
      child: RadioListTile<ActionStatusFilter>(
        value: filter,
  ...
  String _getFilterTitle(ActionStatusFilter filter) {
    switch (filter) {
  ...
  String _getFilterDescription(ActionStatusFilter filter) {
    switch (filter) {
      case ActionStatusFilter.all:
        return 'Show all action flows';
      case ActionStatusFilter.pending:
        return 'Show only pending actions';
      case ActionStatusFilter.running:
        return 'Show only running actions';
      case ActionStatusFilter.completed:
        return 'Show only completed actions';
      case ActionStatusFilter.error:
        return 'Show only failed actions';
    }
  }
}
